...
class WatermarkEncoderTest(unittest.TestCase):
    def test_output_shape(self) -> None:
        ...
        messages = torch.randint(0, 2, (4, 16), dtype=torch.float32)
    ...
    def test_architecture(self) -> None:
        encoder = WatermarkEncoder(feature_channels=24)

        self.assertEqual(encoder.conv1.in_channels, 3)
        self.assertEqual(encoder.conv4.out_channels, 24)
        self.assertEqual(encoder.conv5.in_channels, 24 + 16 + 3 + 1)
        self.assertEqual(encoder.conv_out.kernel_size, (1, 1))
        self.assertEqual(
            sum(isinstance(module, nn.BatchNorm2d) for module in encoder.modules()),
            5,
        )
